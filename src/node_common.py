...
async def handle_connection(reader, writer, node_name, node_info_callback=None):
    ...
    while True:
        ...
        if initial_message and node_info_callback:
            await node_info_callback(addr, message, writer)  # Await the coroutine
            initial_message = False
        else:
            await asyncio.sleep(0.1)  # Add a small delay before sending acknowledgment
            acknowledgment = f"Message received by {node_name}"
...
async def listen_for_messages(reader, node_name):
    while True:
        data = await reader.read(1024)
        if not data:
            break
        print(f"{node_name} received acknowledgment: {data.decode()}")
...
async def start_node(host, port, peer_host, peer_port, node_name, node_info_callback=None):
    ...
    if peer_host and peer_port:
        ...
        asyncio.create_task(listen_for_messages(reader, node_name))
